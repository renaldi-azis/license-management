...
class LoggingTest:
    ...
    def setup_logging_test(self):
        ...
        response = self.session.post(
            ...
            headers={
                "Authorization": "Bearer dummy-token",  # Will be ignored for setup
    ...
    def check_usage_logs(self, expected_count, action_filter=None):
        ...
        db_path = "licenses.db"  # Adjust if needed
        ...
        for i, url in enumerate(test_cases):
            for attempt in range(3 if i == 2 else 1):  # More attempts for rate limiting
                ...
                if response.status_code in [400, 404, 429]:
                    error_count += 1
                    print(f"Generated error {error_count}: {response.status_code} - {response.json().get('error', 'Unknown')}")
    ...
    def test_log_retention(self):
        ...
        try:
            ...
            print("ℹ️  Could not check log files")
